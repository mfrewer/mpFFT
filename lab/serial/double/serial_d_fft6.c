...
void code_info()
{
 ...
 \bFFT-algorithm: DIT, recursive SR-2/4, conjugate-pair, 4m2a, 1-butterfly, recursion-break: n=16, twiddle-index: mul(*)\n",
 CODE,N,IN);
}
...
void print_fft_result()
{
 if(PRA)
 {
  FILE *f=fopen("data_serial_d_fft6.dat","w");
...
void test()
{
 if(PRB)
 {
  FILE *f1=fopen("data_serial_d_fft6_test.dat","w"),
       *f2=fopen("data_serial_d_fft6_test_error.dat","w");
