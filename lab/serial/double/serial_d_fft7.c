...
void code_info()
{
 ...
 \bFFT-algorithm: DIT, recursive SR-2/4, conjugate-pair, 4m2a, 1-butterfly, recursion-break: n=16, twiddle-index: shift(<<)\n",
 CODE,N,IN);
}
...
void d_sr24_cp_fft_v2(unsigned long n, unsigned long log2n, unsigned long s, long int sn,
                      double *inr, double *ini, double *outr, double *outi)
{
 ...
 {
  d_sr24_cp_fft_v2(n/2,log2n+1,2*s,sn,inr,ini,outr,outi);
  d_sr24_cp_fft_v2(n/4,log2n+2,4*s,+1*s+sn,inr+s,ini+s,outr+2*n/4,outi+2*n/4);
  d_sr24_cp_fft_v2(n/4,log2n+2,4*s,-1*s+sn,inr-s,ini-s,outr+3*n/4,outi+3*n/4);
...
void print_fft_result()
{
 if(PRA)
 {
  FILE *f=fopen("data_serial_d_fft7.dat","w");
...
void test()
{
 if(PRB)
 {
  FILE *f1=fopen("data_serial_d_fft7_test.dat","w"),
       *f2=fopen("data_serial_d_fft7_test_error.dat","w");
