...
#endif
#ifndef PAR
#define PAR 1 // parallel environment starts in main
#endif
...
void code_info()
{
 printf("\nParallel: Cores=4, multi-precision, CODE=%d, N=%lu, IN=%d\n\
 \bFFT-algorithm: DIT, recursive SR-2/4, conjugate-pair, 3m3a, 3-butterfly, recursion-break: n=4,\
 openmp (omp task): n>8, twiddle-index: mul(*)\n",CODE,N,IN);
...
// defining and initializing the globals
{
 ...
 LUTs=malloc(N/4*sizeof(mpfr_t)); LUTd=malloc(N/4*sizeof(mpfr_t));
 for(unsigned long k=0;k<N/4;k++) mpfr_inits2(P,LUTr[k],LUTi[k],LUTs[k],LUTd[k],NULL);
...
void lut()
{
 ...
 for(unsigned long k=0;k<N/4;k++)
 {
  ...
  mpfr_add(LUTs[k],LUTr[k],LUTi[k],R); mpfr_sub(LUTd[k],LUTr[k],LUTi[k],R);
 }
}
...
void mp_sr24_cp_fft(unsigned long n, unsigned long s, long int sn, mpfr_t *inr, mpfr_t *ini, mpfr_t *outr, mpfr_t *outi)
{
 ...
 else
 {
  ...
  mpfr_add(a1r,outr[2*n/4],outr[3*n/4],R);
  mpfr_sub(a2r,outr[2*n/4],outr[3*n/4],R);

  mpfr_add(a1i,outi[2*n/4],outi[3*n/4],R);
  mpfr_sub(a2i,outi[2*n/4],outi[3*n/4],R);

  mpfr_sub(outr[2*n/4],outr[0*n/4],a1r,R);
  mpfr_add(outr[0*n/4],outr[0*n/4],a1r,R);
  mpfr_sub(outr[3*n/4],outr[1*n/4],a2i,R);
  mpfr_add(outr[1*n/4],outr[1*n/4],a2i,R);

  mpfr_sub(outi[2*n/4],outi[0*n/4],a1i,R);
  mpfr_add(outi[0*n/4],outi[0*n/4],a1i,R);
  mpfr_add(outi[3*n/4],outi[1*n/4],a2r,R);
  mpfr_sub(outi[1*n/4],outi[1*n/4],a2r,R);

  if(CC){cmul+=0; cadd+=12; cfc+=0; crb+=0;}

  // general butterfly for all k, extracting k=n/8 as special butterfly

  // start: serial routine below a certain value n>4, to reduce openmp overhead
  if(n<32)
  {
   unsigned long k,kn;
   for(k=1;k<n/4;k++)
   {
    ...
    if(k==n/8)
    {
     mpfr_add(m1r,outr[k+2*n/4],outi[k+2*n/4],R); mpfr_mul(m1r,m1r,LUTr[kn],R);
     mpfr_sub(m1i,outi[k+2*n/4],outr[k+2*n/4],R); mpfr_mul(m1i,m1i,LUTr[kn],R);
     mpfr_sub(m2r,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(m2r,m2r,LUTr[kn],R);
     mpfr_add(m2i,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(m2i,m2i,LUTr[kn],R);

     if(CC){cmul+=4; cadd+=4; cfc+=0; crb+=0;}
    }
    else
    {
     mpfr_mul(m1r,LUTd[kn],outi[k+2*n/4],R);
     mpfr_mul(m2r,LUTd[kn],outi[k+3*n/4],R);
     mpfr_mul(m1i,LUTs[kn],outr[k+2*n/4],R);
     mpfr_mul(m2i,LUTs[kn],outr[k+3*n/4],R);

     mpfr_add(tmp,outr[k+2*n/4],outi[k+2*n/4],R); mpfr_mul(tmp,tmp,LUTr[kn],R);
     mpfr_sub(m1r,tmp,m1r,R); mpfr_sub(m1i,tmp,m1i,R);

     mpfr_sub(tmp,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(tmp,tmp,LUTr[kn],R);
     mpfr_add(m2r,tmp,m2r,R); mpfr_sub(m2i,m2i,tmp,R);

     if(CC){cmul+=6; cadd+=6; cfc+=0; crb+=0;}
    }
  ...
  // start: parallel routine (defined for n>=16)
  else
  {
   // parallel task 1: loop k=[1,1*n/16[; range outside k=n/8 of special butterfly
   #pragma omp task
   {
    // declaring private variables
    unsigned long k,kn,ks=N/n;
    mpfr_t _tmp,_m1r,_m1i,_m2r,_m2i,_a1r,_a1i,_a2r,_a2i;
    mpfr_inits2(P,_tmp,_m1r,_m1i,_m2r,_m2i,_a1r,_a1i,_a2r,_a2i,NULL);

    for(k=1;k<(1*n/16);k++)
    {
     kn=k*ks;

     mpfr_mul(_m1r,LUTd[kn],outi[k+2*n/4],R);
     mpfr_mul(_m2r,LUTd[kn],outi[k+3*n/4],R);
     mpfr_mul(_m1i,LUTs[kn],outr[k+2*n/4],R);
     mpfr_mul(_m2i,LUTs[kn],outr[k+3*n/4],R);

     mpfr_add(_tmp,outr[k+2*n/4],outi[k+2*n/4],R); mpfr_mul(_tmp,_tmp,LUTr[kn],R);
     mpfr_sub(_m1r,_tmp,_m1r,R); mpfr_sub(_m1i,_tmp,_m1i,R);

     mpfr_sub(_tmp,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(_tmp,_tmp,LUTr[kn],R);
     mpfr_add(_m2r,_tmp,_m2r,R); mpfr_sub(_m2i,_m2i,_tmp,R);

     mpfr_add(_a1r,_m1r,_m2r,R);
     mpfr_sub(_a2r,_m1r,_m2r,R);

     mpfr_add(_a1i,_m1i,_m2i,R);
     mpfr_sub(_a2i,_m1i,_m2i,R);

     mpfr_sub(outr[k+2*n/4],outr[k+0*n/4],_a1r,R);
     mpfr_add(outr[k+0*n/4],outr[k+0*n/4],_a1r,R);
     mpfr_sub(outr[k+3*n/4],outr[k+1*n/4],_a2i,R);
     mpfr_add(outr[k+1*n/4],outr[k+1*n/4],_a2i,R);

     mpfr_sub(outi[k+2*n/4],outi[k+0*n/4],_a1i,R);
     mpfr_add(outi[k+0*n/4],outi[k+0*n/4],_a1i,R);
     mpfr_add(outi[k+3*n/4],outi[k+1*n/4],_a2r,R);
     mpfr_sub(outi[k+1*n/4],outi[k+1*n/4],_a2r,R);

     if(CC){
     #pragma omp critical
     {cmul+=6; cadd+=18; cfc+=0; crb+=0;}}
    }

    mpfr_clears(_tmp,_m1r,_m1i,_m2r,_m2i,_a1r,_a1i,_a2r,_a2i,NULL);
    mpfr_free_cache();
   } // end: parallel task 1

   // parallel task 2: loop k=[1*n/16,2*n/16[; range outside k=n/8 of special butterfly
   #pragma omp task
   {
    // declaring private variables
    unsigned long k,kn,ks=N/n;
   ...
   // parallel task 3: loop k=[2*n/16,3*n/16[; range inside k=n/8 of special butterfly
   #pragma omp task
   {
    // declaring private variables
    unsigned long k,kn,ks=N/n;
    ...
    for(k=(2*n/16);k<(3*n/16);k++)
    {
     kn=k*ks;

     if(k==n/8)
     {
      mpfr_add(_m1r,outr[k+2*n/4],outi[k+2*n/4],R); mpfr_mul(_m1r,_m1r,LUTr[kn],R);
      mpfr_sub(_m1i,outi[k+2*n/4],outr[k+2*n/4],R); mpfr_mul(_m1i,_m1i,LUTr[kn],R);
      mpfr_sub(_m2r,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(_m2r,_m2r,LUTr[kn],R);
      mpfr_add(_m2i,outr[k+3*n/4],outi[k+3*n/4],R); mpfr_mul(_m2i,_m2i,LUTr[kn],R);

      if(CC){
      #pragma omp critical
      {cmul+=4; cadd+=4; cfc+=0; crb+=0;}}
     }
     else
     {
      mpfr_mul(_m1r,LUTd[kn],outi[k+2*n/4],R);
   ...
   // parallel task 4: loop k=[3*n/16,4*n/16[; range outside k=n/8 of special butterfly
   #pragma omp task
   {
    // declaring private variables
    unsigned long k,kn,ks=N/n;
...
// print fft-result in double precision
{
 if(PRA)
 {
  FILE *f=fopen("data_parallel_mp_fft3_d.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
// print to arbitrary decimal precision (Q<=P)*log(10,2), in the format: %[width].[precision]Re
{
 if(PRB)
 {
  FILE *f=fopen("data_parallel_mp_fft3_q.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
void test()
{
 if(PRC)
 {
  FILE *f1=fopen("data_parallel_mp_fft3_test.dat","w"),
       *f2=fopen("data_parallel_mp_fft3_test_error.dat","w");
...
void clear_memory()
{
 ...
 free(LUTr); free(LUTi); free(LUTs); free(LUTd);
 mpfr_free_cache();
}
