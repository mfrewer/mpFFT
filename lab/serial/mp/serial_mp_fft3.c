...
void code_info()
{
 ...
 \bFFT-algorithm: DIT, recursive SR-2/4, conjugate-pair, 3m3a, 3-butterfly, recursion-break: n=4, twiddle-index: mul(*)\n",
 CODE,N,IN);
...
// print fft-result in double precision
{
 if(PRA)
 {
  FILE *f=fopen("data_serial_mp_fft3_d.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
// print to arbitrary decimal precision (Q<=P)*log(10,2), in the format: %[width].[precision]Re
{
 if(PRB)
 {
  FILE *f=fopen("data_serial_mp_fft3_q.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
void test()
{
 if(PRC)
 {
  FILE *f1=fopen("data_serial_mp_fft3_test.dat","w"),
       *f2=fopen("data_serial_mp_fft3_test_error.dat","w");
