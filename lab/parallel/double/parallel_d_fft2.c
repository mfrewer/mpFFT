...
void code_info()
{
 printf("\nParallel: Cores=2, double-precision, CODE=%d, N=%lu, IN=%d\n\
...
void d_sr24_cp_fft(unsigned long n, unsigned long s, long int sn, double *inr, double *ini, double *outr, double *outi)
{
 ...
 else
 {
  ...
  #pragma omp parallel for firstprivate(ks) if(n>128) num_threads(2)
  for(k=0;k<n/4;k++)
  {
   kn=k*ks;
...
void print_fft_result()
{
 if(PRA)
 {
  FILE *f=fopen("data_parallel_d_fft2.dat","w");
...
void test()
{
 if(PRB)
 {
  FILE *f1=fopen("data_parallel_d_fft2_test.dat","w"),
       *f2=fopen("data_parallel_d_fft2_test_error.dat","w");
