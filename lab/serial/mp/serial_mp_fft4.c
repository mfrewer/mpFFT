...
void code_info()
{
 ...
 \bFFT-algorithm: DIT, recursive SR-2/4, conjugate-pair, 3m3a, 3-butterfly, recursion-break: n=4, twiddle-index: shift(<<)\n",
 CODE,N,IN);
...
void mp_sr24_cp_fft_v2(unsigned long n, unsigned long log2n, unsigned long s, long int sn,
                       mpfr_t *inr, mpfr_t *ini, mpfr_t *outr, mpfr_t *outi)
{
 ...
 {
  mp_sr24_cp_fft_v2(n/2,log2n+1,2*s,sn,inr,ini,outr,outi);
  mp_sr24_cp_fft_v2(n/4,log2n+2,4*s,+1*s+sn,inr+s,ini+s,outr+2*n/4,outi+2*n/4);
  mp_sr24_cp_fft_v2(n/4,log2n+2,4*s,-1*s+sn,inr-s,ini-s,outr+3*n/4,outi+3*n/4);
...
// print fft-result in double precision
{
 if(PRA)
 {
  FILE *f=fopen("data_serial_mp_fft4_d.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
// print to arbitrary decimal precision (Q<=P)*log(10,2), in the format: %[width].[precision]Re
{
 if(PRB)
 {
  FILE *f=fopen("data_serial_mp_fft4_q.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
void test()
{
 if(PRC)
 {
  FILE *f1=fopen("data_serial_mp_fft4_test.dat","w"),
       *f2=fopen("data_serial_mp_fft4_test_error.dat","w");
