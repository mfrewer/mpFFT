...
void code_info()
{
 ...
 openmp (omp for): n>8, twiddle-index: mul(*)\n",CODE,N,IN);
...
void mp_sr24_cp_fft(unsigned long n, unsigned long s, long int sn, mpfr_t *inr, mpfr_t *ini, mpfr_t *outr, mpfr_t *outi)
{
 ...
 else
 {
  ...
  // start: serial routine below a certain value n, to reduce openmp overhead
  if(n<16)
  {
   unsigned long k,kn;
   for(k=1;k<n/4;k++)
  ...
  else
  {
   ...
   #pragma omp parallel private(kn) firstprivate(ks) num_threads(4)
   {
...
// print fft-result in double precision
{
 if(PRA)
 {
  FILE *f=fopen("data_parallel_mp_fft2_d.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
// print to arbitrary decimal precision (Q<=P)*log(10,2), in the format: %[width].[precision]Re
{
 if(PRB)
 {
  FILE *f=fopen("data_parallel_mp_fft2_q.dat","w");
  for(unsigned long k=0;k<N;k++)
  {
...
void test()
{
 if(PRC)
 {
  FILE *f1=fopen("data_parallel_mp_fft2_test.dat","w"),
       *f2=fopen("data_parallel_mp_fft2_test_error.dat","w");
